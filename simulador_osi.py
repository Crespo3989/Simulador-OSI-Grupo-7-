...
class OSISimulator(tk.Tk):
    ...
    def _escribiendo(self):
        """True si el foco esta en un campo de texto (mensaje, IP, MAC, TTL...)."""
        try:
            return isinstance(self.focus_get(), (tk.Text, tk.Entry))
        except (KeyError, tk.TclError):
            return False

    def _keys(self):
        # Los atajos no se disparan mientras se escribe en un campo de texto
        atajo = lambda accion: (lambda e: None if self._escribiendo() else accion())
        self.bind("<space>", atajo(self.toggle_play))
        self.bind("<Right>", atajo(self.siguiente))
        self.bind("<Left>", atajo(self.anterior))
        self.bind("<r>", atajo(self.reiniciar))
        self.bind("<R>", atajo(self.reiniciar))
        # Al hacer clic fuera de un campo de texto, este pierde el foco y los
        # atajos vuelven a funcionar
        self.bind_all("<Button-1>", self._quitar_foco, add="+")

    def _quitar_foco(self, e):
        w = e.widget
        if isinstance(w, tk.Misc) and not isinstance(w, (tk.Text, tk.Entry)):
            w.focus_set()
